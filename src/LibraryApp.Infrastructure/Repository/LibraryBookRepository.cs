...
public class LibraryBookRepository : RepositoryBase, ILibraryBookRepository
{
    ...
    {
        LibraryBookApiModel LibraryBookApiModel = new();
        using (var connection = this.OpenConnection())
        {
            var result = connection.Query<LibraryBookApiModel>(@"SELECT C.LibraryBookCode, C.ISBN, C.Title, C.Author, C.IsStolen, C.IsLost, C.CopyNumber,
                         C.DateCreated, C.CreatedBy, C.DateModified, C.ModifiedBy
                         FROM LibraryBook C
	                     WHERE C.LibraryBookCode = @LibraryBookCode", new { LibraryBookCode });

            if(result != null)
            {
                LibraryBookApiModel = result.SingleOrDefault() ?? new LibraryBookApiModel();
            }
    ...
    public virtual LibraryBookPageApiModel GetLibraryBooksPaged(PagedBase filterParameters, bool listLostAndStolen, out int searchResultCount)
    {
        ...
        searchResultCount = 0;
        IEnumerable<LibraryBookListItem> LibraryBooks = default!;
    ...
    /// <param name="LibraryBookCode"></param>
    /// <returns>The<see cref="int"/> </returns>
    public virtual int InsertLibraryBook(LibraryBookApiModel LibraryBook, out string LibraryBookCode, TransactionParam transactionParam = default!)
    {
